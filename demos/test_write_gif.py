...
def make_frame(size, frame_idx, total_frames, bg=(255, 255, 255, 255)):
    ...
    x0 = int((0.1 + 0.8 * t) * w)
    dr.rectangle(
        [x0 - max(2, w // 20), 0, x0 + max(2, w // 20), h],
        fill=(0, 0, 0, 40),
    )
...
def cleanup_frames(paths_a, paths_b):
    """
    Delete exactly the PNGs we created, then remove their parent folders if empty.
    Leaves GIFs intact.
    """
...
def main():
    ...
    # 1) Basic usage: explicit list input
    write_gif(
    ...
    # 2) Uniform canvas: explicit list input
    write_gif(
    ...
    # 3) Duration override using explicit list
    write_gif(
    ...
    # 4) Wildcard string input with the new write_gif behavior
    frames_b_pattern = str(Path(paths_b[0]).parent / "frame_*.png")
    write_gif(
        gif_name=str(figpath("tests/out/gif_glob.gif")),
        frames=frames_b_pattern,
        fps=15,
        sort_frames=True,
        uniform_size=True,
    )

    # 5) Wildcard string on unpadded filenames to show natural sorting still works
