...
def ellipse_points_3d(p1, p2, eccentricity, inclination=0.0, num_points=200):
...
if __name__ == "__main__":
    ...
    pts = ellipse_points_3d(p1, p2, e, theta)

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(pts[0], pts[1], pts[2], label=f'Ellipse (e={e})')
    ax.scatter([p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], color='r', label='Endpoints')

    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.legend()
    ax.set_title('3‑D Ellipse Through Two Points with Eccentricity and Inclination')
    plt.tight_layout()
    plt.axis('equal')
    plt.show()
