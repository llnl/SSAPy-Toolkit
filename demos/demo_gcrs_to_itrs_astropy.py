...
def main(verbose=True):
    ...
    ----------
    verbose : bool
        If True, print the transformed state and magnitudes.

    Returns
    -------
    dict
        Original and transformed position/velocity objects.
    """
    # Define example GCRS coordinates and velocities
    gcrs_x = 1.0 * u.m
    gcrs_y = 2.0 * u.m
    gcrs_z = 3.0 * u.m
    ...
    # Create a CartesianRepresentation for position
    position = CartesianRepresentation(gcrs_x, gcrs_y, gcrs_z)

    # Create a CartesianDifferential for velocity
    ...
    if verbose:
        ...
        print(
            "Magnitudes:",
            np.linalg.norm([gcrs_x.value, gcrs_y.value, gcrs_z.value]),
            np.linalg.norm([itrs_position.x.value, itrs_position.y.value, itrs_position.z.value]),
        )

    return {
        "obstime": obstime,
        "gcrs_position": position,
        "gcrs_velocity": velocity,
        "gcrs_coords": gcrs_coords,
        "itrs_coords": itrs_coords,
        "itrs_position": itrs_position,
        "itrs_velocity": itrs_velocity,
    }
