...
import os
import sys
from yeager_utils import RGEO, EARTH_RADIUS, ssapy_orbit, make_white, make_black, draw_dashed_circle, create_sphere, koe_plot, orbit_plot, tracking_plot, globe_plot, save_plot_to_pdf, koe_2dhist, display_figure, save_plot, check_numpy_array, groundTrackPlot


satellite_cache = None
satellite_cache2 = None


def imname(filename):
    save_dir = f"{os.path.dirname(os.path.abspath(sys.argv[0]))}/images/"
...
def test_koe_2dhist():
    ...
    fig = koe_2dhist(stable_data, save_path=imname("koe_2dhist"))
...
def test_tracking_plot_single_satellite(satellite_data):
    ...
    fig = tracking_plot(r, t, title="Single Satellite Tracking", save_path=imname("tracking_single_sat"))
...
def test_tracking_plot_multiple_satellites(satellite_data):
    r, t = satellite_data()
    r2, t = satellite_data2()
    r_all = [r, r2]  # List of satellite positions

    # Call the function with multiple satellites
    fig = tracking_plot(r=r_all, t=t, title="Multiple Satellite Tracking", save_path=imname("tracking_multiple_sats"))
...
def test_tracking_plot_with_ground_stations(satellite_data):
    ...
    fig = tracking_plot(r, t, ground_stations=ground_stations, title="Ground Stations Test", save_path=imname("tracking_ground_stations"))
...
def test_globe_plot():
    ...
    fig, ax_tuple = globe_plot(r, t, limits=limits, title=title, figsize=figsize,
                               el=el, az=az, scale=scale, save_path=imname("globe_plot"))
...
def test_orbit_plot():
    ...
    fig, axes = orbit_plot(r, t, title="Satellite Orbit", figsize=(7, 7), save_path=imname("orbit_plot"), frame="gcrf", show=False)
