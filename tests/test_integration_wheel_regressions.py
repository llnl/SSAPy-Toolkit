...
class WheelConstraintTests(unittest.TestCase):
    ...
    def test_capacity_and_restart_for_all_scipy_methods(self):
        for method in ("RK23", "RK45", "DOP853", "Radau", "BDF", "LSODA"):
            with self.subTest(method=method):
                options = self.options(method=method)
                tr = propagate_6dof(times=[0, 1], **options)
                ...
                restarted = propagate_6dof(
                    ...
                    wheel_momentum0=tr.wheel_momentum[-1], **options,
                )
    ...
    def test_reversed_command_desaturates_without_lost_momentum(self):
        ...
        # Integral of the piecewise linear body rate is 0.0075 rad at t=2.
    ...
    def test_saturation_with_mass_and_omega_dependent_inertia(self):
        def inertia(t, r, v, q, omega, *, mass=None):
            return (mass + np.dot(omega, omega)) * np.eye(3)

        tr = propagate_6dof(
            times=np.linspace(0, 1, 21), dense_output=True,
            **self.options(inertia=inertia, mass0=2, mass_flow_rate=lambda *args: 0.1,
                           omega0=[0, 0, 0.1], rtol=1e-11),
        )
        self.assertTrue(np.all(np.abs(tr.wheel_momentum) <= 0.01))
        h = (tr.mass + tr.omega[:, 2]**2) * tr.omega[:, 2] + tr.wheel_momentum[:, 0]
        np.testing.assert_allclose(h, 2.01 * 0.1, rtol=0, atol=3e-10)
        dense = tr.solution(np.linspace(0, 1, 121))
        self.assertTrue(np.all(np.abs(dense[14]) <= 0.01))
        np.testing.assert_allclose((dense[13] + dense[12]**2) * dense[12] + dense[14],
                                   2.01 * 0.1, rtol=0, atol=3e-10)
    ...
    def test_variational_saturation_includes_change_in_hit_time(self):
        ...
        # h reaches -0.01 at 0.55 s. A perturbation in initial h advances
        # that time by delta_h / torque; it changes both final rate and angle.
        hit = 0.55
