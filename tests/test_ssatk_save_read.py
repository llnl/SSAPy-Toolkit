...
from ssapy_toolkit.io.ssatk_save import ssatk_read, ssatk_save, supported_save_formats
...
def test_ssatk_save_read_hdf5_defaults_keys_and_roots(tmp_path, monkeypatch):
    ...
    np.testing.assert_array_equal(ssatk_read(single_path), np.arange(4))

    ssatk_save(np.arange(2), single_path, key="orbit/r", overwrite=True)
    np.testing.assert_array_equal(ssatk_read(single_path, key="orbit/r"), np.arange(2))
...
def test_ssatk_save_read_numpy_json_csv_text_and_pickle(tmp_path, monkeypatch):
    ...
    np.testing.assert_array_equal(ssatk_read(npz_path, key="orbit/r"), [1, 2])

    json_path = ssatk_save({"arr": np.arange(2), "label": "ok"}, "payload.json")
    loaded_json = ssatk_read(json_path)
    ...
    pd.testing.assert_frame_equal(ssatk_read(csv_path), frame)

    text_path = ssatk_save(["alpha", "beta"], "notes.txt")
    assert ssatk_read(text_path) == "alpha\nbeta\n"

    pickle_path = ssatk_save({"items": [object()]}, tmp_path / "explicit.pkl")
    assert "items" in ssatk_read(pickle_path)
...
def test_ssatk_save_read_jsonl_npz_pickle_safety_and_top_level_aliases(tmp_path, monkeypatch):
    ...
    assert ssatk.ssatk_read is ssatk_read
