...
class LeapfrogImpactTests(unittest.TestCase):
    def test_grazing_enter_exit_is_detected_with_physical_speed(self):
        ...
        # A local ballistic chord isolates detection from force approximation.
    ...
    def test_elapsed_profile_bounds_with_absolute_callback_epochs(self):
        epochs = []
        def model(r, t):
            epochs.append(t)
            return np.zeros(3)
        with patch('ssapy_toolkit.propagators_orbit.leap_frog.accel_point_earth',
                   return_value=np.zeros(3)):
            _, v = leapfrog([7e6, 0, 0], [1, 0, 0], 1.4e9 + np.arange(3),
                            radial={'start_time': 0, 'end_time': 1, 'accel': 2},
                            accels=model)
        self.assertEqual(v[1, 0], 2.)
        self.assertTrue(all(1.4e9 <= t <= 1.4e9 + 2 for t in epochs))
