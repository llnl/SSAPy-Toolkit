...
def acceleration_adapter(model, signature=None):
    """Return a ``(t, r, v)`` adapter without probing a model by executing it.

    Explicit signatures: ``r``, ``v``, ``rt``, ``tr``, ``rv``, ``rvt``, ``trv``.
    Supply ``signature`` here or set ``model.acceleration_signature``. Without
    either, positional parameters must use the canonical names r, v, and
    t/time/epoch (leading underscores are ignored). Other names, variadic
    callables and non-inspectable callables require an explicit declaration.
    Bind force parameters such as magnitude with functools.partial first.

    Resolution happens once when a propagator constructs its model list.
    Exceptions from inside a callback always propagate without signature retries.
    """
    ...
    if parameters is not None:
        # This detects argument mismatches without catching errors in the model.
