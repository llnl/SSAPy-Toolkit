...
def transfer_shooter(*args, r1=None, v1=None, r2=None, v2=None, orbit1=None, orbit2=None, tol=1, max_iter=50, plot=False, status=False, accel=AccelKepler()):
    ...
    *args : tuple
    ...
    if plot and len(r_transfer) >= 2 and len(v_transfer) >= 2 and result['tof'] > 0:
        ...
        fig = transfer_plot(
            ...
            v2,
            title=(
                f"Transfer time: {result['tof'] / 60:.0f} min\n"
                f"|Δv₁| {np.linalg.norm(delta_v) / 1e3:.3f}, "
                f"|Δv₂| {np.linalg.norm(v2 - v_transfer[closest_idx]) / 1e3:.3f} km/s"
            ),
            show=False,
        )
        result['fig'] = fig
    elif plot and status:
        print("Skipping transfer_plot: transfer trajectory is degenerate or tof <= 0.")
