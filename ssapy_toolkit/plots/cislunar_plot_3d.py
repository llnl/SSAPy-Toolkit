...
def cislunar_plot_3d(r, t=None, figsize=(8, 8), fontsize=12, save_path=False, show=False, legend=True, title='', c='black'):
    """
    Author: Travis Yeager (yeager7@llnl.gov)
    Enhanced with star background.
    """
    ...
    # Clean up panes for dark background
    if dark_bg:
    ...
    for orbit_index in range(len(r)):
        ...
        lower_bound_lunar_temp, upper_bound_lunar_temp = find_smallest_bounding_cube(
            xyz_lunar, pad=1)
        ...
        mask_lunar = (
            (r_earth_lunar[:, 0] >= bounds_lunar["lower"][0]) &
            (r_earth_lunar[:, 0] <= bounds_lunar["upper"][0]) &
            (r_earth_lunar[:, 1] >= bounds_lunar["lower"][1]) &
            (r_earth_lunar[:, 1] <= bounds_lunar["upper"][1]) &
            (r_earth_lunar[:, 2] >= bounds_lunar["lower"][2]) &
            (r_earth_lunar[:, 2] <= bounds_lunar["upper"][2])
        )

        if np.size(r_moon[:, 0]) > 1:
            blue_colors = cm.Blues(
                np.linspace(0.2, .8, len(r_moon[:, 0])))[::-1][mask_lunar]
        else:
            blue_colors = "blue"
        ...
        ax.scatter3D(xyz_lunar[:, 0], xyz_lunar[:, 1], xyz_lunar[:, 2],
                     color=scatter_dot_colors, s=1)

        moon_r   = MOON_RADIUS / unit_conversion
        temp_range = max(
            bounds_lunar["upper"][0] - bounds_lunar["lower"][0],
            bounds_lunar["upper"][1] - bounds_lunar["lower"][1],
            bounds_lunar["upper"][2] - bounds_lunar["lower"][2],
        ) if bounds_lunar["upper"][0] != -np.inf else moon_r * 50
        visible_moon_r = max(moon_r, temp_range * 0.04)
        moon_img = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                '..', '..', '..', 'SSAPy', 'ssapy', 'data', 'moon.png'))
        from .orbit_plot_xy import _textured_sphere
        drawn = _textured_sphere(ax, 0, 0, 0, visible_moon_r, moon_img, n=48)
        if not drawn:
            mesh_x = np.outer(np.cos(u), np.cos(v)).T * visible_moon_r
            mesh_y = np.outer(np.sin(u), np.cos(v)).T * visible_moon_r
            mesh_z = np.outer(np.ones(np.size(u)), np.sin(v)).T * visible_moon_r
            ax.plot_surface(mesh_x, mesh_y, mesh_z, color="grey", alpha=0.6, edgecolor='none')

        ax.scatter3D(
            r_earth_lunar[mask_lunar, 0],
            r_earth_lunar[mask_lunar, 1],
            r_earth_lunar[mask_lunar, 2],
            color=blue_colors, s=55)
        ...
        for (point, pos) in lagrange_points_lunar_fixed_frame().items():
            ...
            if (bounds_lunar["lower"][0] <= pos[0] <= bounds_lunar["upper"][0] and
                    bounds_lunar["lower"][1] <= pos[1] <= bounds_lunar["upper"][1] and
                    bounds_lunar["lower"][2] <= pos[2] <= bounds_lunar["upper"][2]):
                ax.scatter(pos[0], pos[1], pos[2],
                           color=textcolor, label=point, s=10)
    ...
    rainbow_line = Line2D([0], [0], color='w',
                          linestyle='-', linewidth=2, label='Orbit Path')
    legend_elements = [
        Patch(facecolor='lightblue', edgecolor=textcolor, label='Earth'),
        Patch(facecolor='lightgrey', edgecolor=textcolor, label='Moon'),
        Line2D([0], [0], marker='o', color='none',
               markerfacecolor='black', markersize=6, label='Lagrange Points'),
    ]
    if len(r) == 1:
