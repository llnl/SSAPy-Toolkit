...
def propagate_spacecraft_segments(spacecraft, segments, **defaults):
    ...
    Event results are grouped by callable identity in first-appearance order;
    ``trajectory.event_functions`` identifies each slot. Reuse the same callable
    to accumulate occurrences across segments. Distinct callables remain distinct
    even when they occupy the same index in different segments. Occurrence-count
    terminal events count independently within each integration segment.
    """
...
def _propagate_spacecraft_segment(spacecraft, segment, *, tracks_mass=False):
    ...
    # The accepted ULP-scale discrepancy represents the same boundary. Snap the
    # sample to it so the strict forward-time validator and dense output agree.
    times = times.copy()
    ...
    final_inertia = (None if trajectory.mass is not None and _supports_body_mass_update(current.body)
...
def _combine_event_results(trajectories):
    """Merge occurrences by identity; unknown legacy slots remain independent."""
    ...
    for trajectory in trajectories:
        for local_index, event_times in enumerate(trajectory.t_events or ()):
            ...
            for epoch, state in zip(event_times, event_states):
                # A continuous event at a shared boundary can be reported by
                # both solvers. Preserve same-epoch states across an impulse.
